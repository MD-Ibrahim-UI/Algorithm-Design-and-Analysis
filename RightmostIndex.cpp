...
int main()
{
    ...
    {                                           // Checks if left index is less than or equal to right index
        ...
        {                        // Checks if the value of mid index is equal to x
            ...
            leftIndex = mid + 1; // The left index will be next index of mid
        }
        else if (b[mid] > x)
    ...
    if (index == -1)
    { // If the value of index is -1, x doesn't exist in this array
        cout << "Element isn't found\n";
    }
    else
    { // If the value of index is anything other than -1, x exists in this array
        cout << "The rightmost index of " << x << " is " << index;
    }
